...
local function get_priority(op)
  ...
  if op.action and op.action.kind then
    local kind = op.action.kind:lower()
    if kind:match("^quickfix%.import") then
      return 0
    end
  end
  if op.title then
    local title = op.title:lower()
    if title:match("^import") or title:match("^add") or title:match("^make") or title:match("^replace") then
      return 0
    elseif title:match("^ignore") then
...
function M.on_code_action_results(results, opts)
  ...
  table.sort(actions, function(a, b)
    ...
    -- Prefer isPreferred actions
    if a.action.isPreferred and not b.action.isPreferred then
      return true
    elseif b.action.isPreferred and not a.action.isPreferred then
      return false
    end

    -- Fallback: sort by kind, then title
